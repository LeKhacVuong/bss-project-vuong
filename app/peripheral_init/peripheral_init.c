...
static void debug_receive_handle_impl(UART_hw* p_hw){
	...
	if(selex_bss_app.slave_com->state == RS485_MASTER_ST_MOBUS){
		selex_bss_app.bss.ac_meter.rx_packet[selex_bss_app.bss.ac_meter.rx_index] = data;
		selex_bss_app.bss.ac_meter.rx_index++;
		if(selex_bss_app.bss.ac_meter.rx_index >= 30){
			selex_bss_app.bss.ac_meter.rx_index = 0;
		}
	}
...
static void rs485_parse_slave_msg_handle_impl(RS485_Master* p_485m){  //@vuong doc tu cabinet
	...
	if(p_485m->csv.id == string_to_long(token)){
		...
		case MASTER_READ:
			...
			token = strtok(NULL, ",");
			selex_bss_app.bss.cabs[p_485m->csv.id].charger.state = string_to_long(token);         //@vuong charger state
			token = strtok(NULL, ",");
			...
			selex_bss_app.bss.cabs[p_485m->csv.id].pogos_temp[0] = string_to_long(token);
			token = strtok(NULL, ",");
			selex_bss_app.bss.cabs[p_485m->csv.id].temp[2] = string_to_long(token);
			selex_bss_app.bss.cabs[p_485m->csv.id].pogos_temp[1] = string_to_long(token);
			token = strtok(NULL, ",");
			selex_bss_app.bss.cabs[p_485m->csv.id].temp[3] = string_to_long(token);
			selex_bss_app.bss.cabs[p_485m->csv.id].pogos_temp[2] = string_to_long(token);
			token = strtok(NULL, ",");
			selex_bss_app.bss.cabs[p_485m->csv.id].temp[4] = string_to_long(token);
			selex_bss_app.bss.cabs[p_485m->csv.id].pogos_temp[3] = string_to_long(token);
