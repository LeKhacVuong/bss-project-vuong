...
extern Cabinet 		bss_cabinets[CABINET_INIT];
	...
	switch (p_ca->hmi_csv.main_obj[msg_id]) {
		case BSS_STATION:
			if (p_ca->hmi_csv.sub_obj[msg_id] == ALL) {
				bss_data_serialize(&p_ca->bss, tx_buff);    //@vuong nơi sắp xếp thông tin vào tx_buff
				cab_app_send_msg_to_hmi(p_ca);
			} else {
		...
		case BSS_CABINET:
			...
			break;
		case BSS_BP:
			bp_data_serialize(p_ca->bss.cabs[p_ca->hmi_csv.id[msg_id]].bp, tx_buff);      //@vuong data BP trong trạm
			cab_app_send_msg_to_hmi(p_ca);
			break;
		default:
			break;
	...
	switch (sub_obj) {
		...
		case POWER_METER:
			if(state == 1){
					...
					selex_bss_app.bss.ac_meter.timeout = *timestamp1 + 1500;
					p_ca->slave_com->state = RS485_MASTER_ST_MOBUS;
					mobus_master_command_serialize(p_ca->slave_com,7);
					HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, 1);
					HAL_UART_Transmit(&debug_com.uart_module , p_ca->slave_com->tx_data, 8, 2000);
					HAL_GPIO_WritePin(GPIOB, GPIO_PIN_12, 0);
					while(selex_bss_app.bss.ac_meter.finish_read == false);
					uint16_t crc;
					crc = MODBUS_CRC16(p_ca->bss.ac_meter.rx_packet,29);
					if(crc == 0){
						bss_update_vse11_metter(&p_ca->bss);
						p_ca->hmi_csv.obj_state[msg_id] = STATE_OK;
					}else{
						p_ca->hmi_csv.obj_state[msg_id] = STATE_FAIL;
					}
...
void cab_app_update_charge(Cabinet_App *p_ca) {
	for (uint8_t id = 0; id < p_ca->bss.charger_num; id++) {    //charger_num = 2
		if (p_ca->bss.ac_chargers[id].charging_cabin != NULL) {    //nhánh chính 1



			if((p_ca->bss.ac_chargers[id].charging_cabin->pogos_temp[0] >= 70) || (p_ca->bss.ac_chargers[id].charging_cabin->pogos_temp[1] >= 70) ||
			   (p_ca->bss.ac_chargers[id].charging_cabin->pogos_temp[2] >= 70) || (p_ca->bss.ac_chargers[id].charging_cabin->pogos_temp[3] >= 70)){
			...
			else if ((p_ca->bss.ac_chargers[id].charging_cabin->bp->vol >= BP_STOP_CHARGER_THRESHOLD) || (charge_no_cur_timestamp[id] >= CHARGER_NO_CURRENT_TIMEOUT)) {
