...
void bss_update_vse11_metter(BSS_Data* p_bss){
	p_bss->ac_meter.ac_voltage = ((uint32_t)p_bss->ac_meter.rx_packet[3] << 24) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[4] << 16) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[5] << 8) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[6]);
	p_bss->ac_meter.total_result = p_bss->ac_meter.ac_voltage;
	p_bss->ac_meter.total_result = p_bss->ac_meter.total_result * 1.00/100;
	p_bss->ac_meter.ac_voltage = ((uint32_t)p_bss->ac_meter.rx_packet[7] << 24) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[8] << 16) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[9] << 8) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[10]);
	p_bss->ac_meter.vol_result = p_bss->ac_meter.ac_voltage;
	p_bss->ac_meter.vol_result = p_bss->ac_meter.vol_result * 1.0/10;
	p_bss->ac_meter.ac_voltage = ((uint32_t)p_bss->ac_meter.rx_packet[11] << 24) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[12] << 16) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[13] << 8) |
								 ((uint32_t)p_bss->ac_meter.rx_packet[14]);
	p_bss->ac_meter.cur_result = p_bss->ac_meter.ac_voltage;
	p_bss->ac_meter.cur_result = p_bss->ac_meter.cur_result * 1.00 / 100;
}
...
Cabinet* bss_get_cab_need_charge(BSS_Data* p_bss, uint8_t charger_id){          //@vuong quét các khoang xem cái nào cần sạc
	...
	for(uint8_t i = 0; i < p_bss->ac_chargers[charger_id].assigned_cab_num; i++){
		...
		if((p_bss->ac_chargers[charger_id].assigned_cabs[i]->bp->vol > 0)
				...
				&& (p_bss->cabs[i].pogos_temp[0] < 61) && (p_bss->cabs[i].pogos_temp[1] < 61)
				&& (p_bss->cabs[i].pogos_temp[2] < 61) && (p_bss->cabs[i].pogos_temp[3] < 61)){
			available_cab_num++;
...
static void bss_data_serialize_impl(BSS_Data* p_bss, char* buff){       //@vuong se goi ham nay de truyen data va bien
	*buff++=':';
	*buff++='R';
    *buff++=',';
	*buff++='S';
...
static void all_data_cabinet_serialize_impl(BSS_Data* p_bss,char* buff){ //bỏ
	*buff++ = ':';
	*buff++ = 'R';
	*buff++ = ',';
	*buff++ = 'C';
